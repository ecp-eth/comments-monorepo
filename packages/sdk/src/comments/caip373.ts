...
export type CAIP373QuotedComment = `eip155:${number}:${Hex}:call:${Hex}`;
...
export function encodeQuotedCommentFromCaip373({
  ...
  return `eip155:${chainId}:${chain.commentManagerAddress}:call:${encodedFunctionCall}${blockNumber ? `:${blockNumber}` : ""}`;
}
