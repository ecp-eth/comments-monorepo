...
async function everyPackage(globPattern, onEachPackage) {
...
async function everyExportWithCJSModule(pkg, onEachExport) {
...
function findProjectRoot() {
  const startPath = nodeProcess.cwd();
...
// package.json
var package_default = {
  name: "@ecp.eth/build-tools",
  version: "0.0.1",
  private: true,
  type: "module",
  bin: {
    "proxy-package": "./bin/index.js"
  },
  scripts: {
    build: "tsup --format esm --out-dir bin/ && chmod +x bin/index.js",
    "proxy-package": "tsx ./src/index.ts"
  },
  dependencies: {
    chalk: "^5.4.1",
    commander: "^13.1.0"
...
program.description("Proxy packages utility").version(package_default.version);
program.command("generate").description("Generate proxy packages").argument("<string>", "The pattern to search for the package").action(async (pattern) => {
  console.log("Generating proxy packages...");
  const results = await everyPackage(pattern, async ({ pkg, path: path2 }) => {
    const dir = nodePath2.resolve(nodePath2.dirname(path2));
    console.log(`Start processing ${pkg.name} at (${dir})`);
    await everyExportWithCJSModule(
      pkg,
      async ({ key, cjsPath, typePath }) => {
        const proxyDir = nodePath2.resolve(dir, key);
        nodeFs2.mkdirSync(proxyDir, { recursive: true });
        const types2 = nodePath2.relative(key, typePath);
        const main = nodePath2.relative(key, cjsPath);
        nodeFs2.writeFileSync(
          `${proxyDir}/package.json`,
          `${JSON.stringify({ type: "module", types: types2, main }, void 0, 2)}
`
        );
...
program.command("hide").description("Hide generated proxy packages in the project").argument("<string>", "The pattern to search for the package").action(async (pattern) => {
  ...
  const results = await everyPackage(pattern, async ({ pkg, path: path2 }) => {
    ...
    await everyExportWithCJSModule(pkg, async ({ key }) => {
      const proxyDir = nodePath2.resolve(dir, key);
      const relativePath = nodePath2.relative(projectRoot, proxyDir);
      const gitignorePath = nodePath2.join(projectRoot, ".gitignore");
      if (!nodeFs2.existsSync(gitignorePath)) {
        nodeFs2.writeFileSync(gitignorePath, "");
      }
      const gitignoreContent = nodeFs2.readFileSync(gitignorePath, "utf-8");
      if (!gitignoreContent.includes(relativePath)) {
        nodeFs2.appendFileSync(gitignorePath, `
${relativePath}`);
        console.log(`Added ${relativePath} to .gitignore`);
      }
      const vscodeSettingsPath = nodePath2.join(
        projectRoot,
        ".vscode/settings.json"
      );
      let settings = {};
      if (nodeFs2.existsSync(vscodeSettingsPath)) {
        settings = JSON.parse(
          nodeFs2.readFileSync(vscodeSettingsPath, "utf-8")
        );
      } else {
        nodeFs2.mkdirSync(nodePath2.dirname(vscodeSettingsPath), {
          recursive: true
        });
      }
