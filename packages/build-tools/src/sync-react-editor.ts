...
// Allow targetDir to be overridden via command-line argument
const targetDirArg = process.argv[2];
const targetDir = resolve(
  resolve(rootDir, targetDirArg ?? "examples/demo-rn-expo/"),
  "node_modules/@ecp.eth/react-editor",
);

function syncReactEditor(targetDir: string) {
  ...
  console.log(
    "🗑️  Step 2: Removing existing react-editor from node_modules...",
  );
  ...
  if (!existsSync(targetParentDir)) {
    throw new Error(
      `Target parent directory does not exist: ${targetParentDir}`,
    );
  }
