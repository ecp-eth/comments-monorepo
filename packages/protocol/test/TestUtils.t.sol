...
contract TestUtilsTest is Test {
     ...
     */
    function testParseEthAmount() pure public {
        // Test case 1: Valid whole number with space
        (bool found1, uint256 amount1) = TestUtils.parseEthAmount(bytes("1 ETH"), 0);
        ...
        (bool found3, uint256 amount3) = TestUtils.parseEthAmount(bytes("0.1 ETH"), 0);
        ...
        (bool found4, uint256 amount4) = TestUtils.parseEthAmount(bytes("0.1ETH"), 0);
        ...
        (bool found5, uint256 amount5) = TestUtils.parseEthAmount(bytes("0.01 ETH"), 0);
        ...
        (bool found10, uint256 amount10) = TestUtils.parseEthAmount(bytes("1 eth"), 0);
        ...
        (bool found11, uint256 amount11) = TestUtils.parseEthAmount(bytes("Here's 1 ETH for you"), 7);
        assertTrue(found11, "Should find amount in middle of string");
        assertEq(amount11, 1 ether, "Should parse 1 ETH from middle of string correctly");

        // Test case 12: Parse with multiple spaces
        (bool found12, uint256 amount12) = TestUtils.parseEthAmount(bytes("1   ETH"), 0);
