...
import { IFeeEstimatableHook } from "../src/interfaces/IFeeEstimatableHook.sol";
import { FeeEstimatableHook } from "../src/hooks/FeeEstimatableHook.sol";

// Fee charging hook contract
contract FlatFeeHook is FeeEstimatableHook {
...
contract FlatFeeHookTest is Test, IERC721Receiver {
  ...
  function test_FlatFeeHookEstimatedAddCommentFeeIsAccurate() public {
    ...
    uint256 estimatedHookFee = feeHook.getAddCommentFee(
      commentData,
      metadata,
      user1,
      bytes32(0)
    ...
    uint256 totalFeeIncludingProtocolFee = protocolFees
      ...
      (estimatedHookFee / (10000 - protocolFees.getHookTransactionFee())) *
      10000;
    ...
    emit log_named_uint("estimatedHookFee", estimatedHookFee);
    emit log_named_uint(
