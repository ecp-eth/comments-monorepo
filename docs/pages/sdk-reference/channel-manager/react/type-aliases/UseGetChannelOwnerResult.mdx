[**@ecp.eth/sdk**](../../../index.mdx)

***

[@ecp.eth/sdk](/sdk-reference/index.mdx) / [channel-manager/react](/sdk-reference/channel-manager/react/index.mdx) / UseGetChannelOwnerResult

# Type Alias: UseGetChannelOwnerResult

```ts
type UseGetChannelOwnerResult = UseQueryResult<GetChannelOwnerResult, Error>;
```

Defined in: [packages/sdk/src/channel-manager/react/channel.ts:196](https://github.com/ecp-eth/comments-monorepo/blob/main/packages/sdk/src/channel-manager/react/channel.ts#L196)
