...
### 4. Appending comment to a transaction (author pays gas)

This approach is similar to the first one, but with a key difference in how the comment is submitted.
Instead of submitting the comment as a separate transaction, the comment is appended to an existing transaction as data suffix.
...
sequenceDiagram
    ...
    App server->>App server: Signs the comment request
    App server-->>Author: Responds comment data and app signature
    Author->>Some contract: Writes to a contract with a comment data as transaction data suffix.
    Some contract->>Indexer: Emit some event
    Indexer->>Indexer: Parses transaction data suffix from the event and stores the comment
    Indexer-->>Author: Responds with updated comments feed
```
