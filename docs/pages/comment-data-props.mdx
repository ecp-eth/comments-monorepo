...
The authentication method can be useful for:

- **Trust indicators** - Display different trust levels in the UI
- **Analytics** - Track which authentication methods are most commonly used
- **Security auditing** - Investigate suspicious comment patterns
- **User experience** - Show users how their comments were authenticated

### Example Usage

```typescript
// Check if a comment was posted directly by the user
if (comment.authMethod === 0) {
  console.log("User posted this comment directly");
}

// Check if comment was posted via pre-approval
if (comment.authMethod === 1) {
  console.log("Comment posted via app pre-approval");
}
```
