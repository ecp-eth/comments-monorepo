...
However if the user has not granted permission, the flow will be slightly more complicatd:
...
First, the client collects the required [comment data](/comment-data-props) and send it to the app server:
...
What's the difference is that the app server will not proceed ahead and submit the transaction.
Because the user previously did not approve the app, we will need the author to sign the comment data,
then post the signed comment back to the app server.

#### Author signs the comment data

The signing of the comment data is automatically handled by the `useGaslessTransaction` hook:
...
In case of attacker utilise the endpoint to drain the gas, you may want to double check the app signature is valid before submitting the transaction:
