...
The React Editor provides a powerful, customizable rich text editor built on top of TipTap that supports mentions, file uploads, and seamless integration with the Ethereum Comments Protocol.
...
const suggestions = useIndexerSuggestions({
  apiUrl: "https://api.ethcomments.xyz", // optional, defaults to https://api.ethcomments.xyz
});
```
