...
description: Integrating ECP comments into React applications using pre-built components.
---
...
#### Showing All Comments (Bypassing Moderation Filter)

When moderation is enabled, only approved comments are shown by default.
Pass `moderationStatus` to include pending or rejected comments:
