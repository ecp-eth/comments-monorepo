...
- For latest contract address, see [COMMENT_MANAGER_ADDRESS](/sdk-reference/defaultExports/variables/COMMENT_MANAGER_ADDRESS).
- We will call [`postCommentAsAuthor()`](/protocol-reference/CommentManager#postcommentasauthorstruct-commentscommentdata-commentdata-bytes-appsignature-external) method to post the comment.
