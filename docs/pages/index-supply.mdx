...
This guide will to walk you through the steps to setup Index Supply to query comments from the protocol.
...
```solidity
CommentAdded(bytes32 indexed commentId, address indexed author, address indexed appSigner, (string content, string metadata, string targetUri, bytes32 parentId, address author, address appSigner, uint256 nonce, uint256 deadline) commentData)
```
