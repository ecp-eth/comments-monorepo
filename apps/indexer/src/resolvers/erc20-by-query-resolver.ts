...
import { ERC20TokensService } from "../services/erc20-tokens-service";
...
export function createERC20ByQueryResolver({
  ...
  return new DataLoader<string, ResolvedERC20Data[]>(async (keys) => {
    return Promise.all(
      keys.map(async (query) => {
        ...
        for (const token of tokens) {
          const client = await clientRegistry.getClientByChainId(token.chainId);
